    ...
    def control_keyboard(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == K_a:
                    if not proper.is_fight_main:
                        if self.contact:
                            self.dialog_group = self.contact[0].Create_dialog()
                            proper.is_player = True
                    else:
                        Fight_Window().main()
                        proper.is_player = False
                        proper.is_fight_main = False
